...
By analyzing the GET and POST from the DVWASession, it is possible to understand that each "security power" has its own id, which is passed to a Script that generates a table with information which the security level is included. About how the cookies are generated, they are set before on the login phase, and it is defined a specific cookie for a user in the site. 
...
The only functional one is the gordonb/abc123, and the only evidence i could find was analyzing the information at the head of each try, and their responses.
...
>docker run --network="host" vanhauser/hydra -V -f -I -l admin -x 1:2:a "http-get://localhost/vulnerabilities/brute/?username=gordonb&password=abc123&Login=Login:H=Cookie\:PHPSESSID=2ujucgqsp6pvpqd4mk5qtuqip2; security=low:F= Connection : keep-alive"
